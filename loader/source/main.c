...
static u32 CheckForMultiGameAndRegion(unsigned int CurDICMD, u32 *ISOShift, u32 *BI2region)
{
	...
	while (true)
	{
		...
		if( redraw )
		{
			...
			{
				const u32 color = gameIsUnaligned[i] ? RED : WHITE;
...
//	u64 timeout = 0;
	...
	*(vu32*)0x932C0494 = CONF_GetSensorBarPosition();
	...
	if(!isWiiVC)
	{
		...
		if (LoadKernel() < 0)
		{
			...
			PrintFormat(DEFAULT_SIZE, RED, MENU_POS_X, MENU_POS_Y + 20*20, "Returning to loader in 10 seconds.");
			UpdateScreen();
			VIDEO_WaitVSync();
	...
	*(vu32*)0x92FFFFC0 = isWiiVC; //cant be detected in IOS
	...
	if (!devices[DEV_SD] && !devices[DEV_USB])
	{
		...
		PrintFormat(DEFAULT_SIZE, RED, MENU_POS_X, 232, "No FAT device found!");
		ExitToLoader(1);
	}
...
//Init DI and set correct ID if needed
	...
	if (ret != 0) {
		ClearScreen();
		PrintInfo();
		PrintFormat(DEFAULT_SIZE, RED, MENU_POS_X, MENU_POS_Y + 20*4, "CheckForMultiGameAndRegion() failed: %d", ret);
		switch (ret) {
			case 1:
				PrintFormat(DEFAULT_SIZE, RED, MENU_POS_X, MENU_POS_Y + 20*5, "Unable to open the %s.",
					CurDICMD == 0 ? "disc image file" : "disc drive");
				break;
			case 2:
				PrintFormat(DEFAULT_SIZE, RED, MENU_POS_X, MENU_POS_Y + 20*5, "Unable to read the disc header.");
				break;
			case 3:
				PrintFormat(DEFAULT_SIZE, RED, MENU_POS_X, MENU_POS_Y + 20*5, "Unable to read the CISO bi2.bin area.");
				break;
			case 4:
				PrintFormat(DEFAULT_SIZE, RED, MENU_POS_X, MENU_POS_Y + 20*5, "Unable to open the extracted FST bi2.bin file.");
				break;
			case 5:
				PrintFormat(DEFAULT_SIZE, RED, MENU_POS_X, MENU_POS_Y + 20*5, "Unable to read the extracted FST bi2.bin file.");
				break;
			default:
				PrintFormat(DEFAULT_SIZE, RED, MENU_POS_X, MENU_POS_Y + 20*5, "Unknown error code.");
				break;
		}
		...
		if (CurDICMD) {
			...
			PrintFormat(DEFAULT_SIZE, RED, MENU_POS_X, MENU_POS_Y + 20*7, "Device: %s", device);
		} else {
			PrintFormat(DEFAULT_SIZE, RED, MENU_POS_X, MENU_POS_Y + 20*7, "Filename: %s:%s", GetRootDevice(), ncfg->GamePath);
		}
	...
	*(vu32*)0xD3003420 = 0x0DEA;
	while(1)
	{
		...
		if(argsboot == false)
		{
			...
			if(STATUS_LOADING == -2)
				PrintFormat(DEFAULT_SIZE, RED, MENU_POS_X, MENU_POS_Y + 20*8, "Initing storage devices... Error! %d  Shutting down", STATUS_ERROR);
			if(STATUS_LOADING == 3)
	...
	/* 			if (timeout == 0)
				...
				PrintFormat(DEFAULT_SIZE, (STATUS_ERROR == -7) ? RED:WHITE, MENU_POS_X, MENU_POS_Y + 20*10, (STATUS_ERROR == -7) ? "Checking FS... Timeout!" : "Checking FS...");
			}
			...
			if(STATUS_LOADING == -8)
			{
				...
				{
					case -1:
						PrintFormat(DEFAULT_SIZE, RED, MENU_POS_X, MENU_POS_Y + 20*15, "No Controller plugged in! %25s", " ");
						break;
					case -2:
					...
					case -6:
						...
						break;
					case -7:
						PrintFormat(DEFAULT_SIZE, RED, MENU_POS_X, MENU_POS_Y + 20*15, "Gamecube adapter for Wii u init error %13s", " ");
						break;
					default:
